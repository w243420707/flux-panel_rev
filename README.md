...
- 新增 `site_traffic` 单例累计计数器，按已过滤的计费流量原子累加入站和出站字节，并与用户、转发、用户隧道流量更新保持事务一致。
- `/api/v1/user/package` 仅在服务端 JWT 角色为管理员时返回 `siteTraffic`；普通用户仍只获得自己的周期用量，自动重置、手动重置和删除数据不会清零站点累计。
- 新装 schema 和既有部署迁移均以迁移时 `user.in_flow/out_flow` 汇总初始化一次站点计数器。
